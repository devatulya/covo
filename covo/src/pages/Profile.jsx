...
export function Profile() {
  ...
  const profileId = id === 'me' ? currentUserId : id;
  const isOwnProfile = !profileId || profileId === currentUserId;
  const [viewUser, setViewUser] = React.useState(user);
  ...
  React.useEffect(() => {
    ...
    apiFetch(`/users/${profileId}`)
      .then(({ user: profile }) => {
        if (!ignore) setViewUser(profile);
      })
      .catch((err) => {
        console.error('Profile load error:', err);
        if (!ignore) setViewUser(null);
      })
      .finally(() => {
  ...
  React.useEffect(() => {
    ...
    const params = new URLSearchParams({ viewerId: currentUserId || '' });
    apiFetch(`/users/${profileId}/posts?${params.toString()}`)
      .then(({ posts }) => {
        if (!ignore) setProfilePosts(posts);
      })
      .catch((err) => {
        console.error('Profile posts load error:', err);
        if (!ignore) setProfilePosts([]);
      })
      .finally(() => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-neoBg pb-20 md:pb-0">
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 p-4 md:p-8">
        {profileLoading ? (
          <div className="surface-panel border-[3px] border-neoBorder p-5 text-center text-sm font-black uppercase text-neoMuted shadow-neo">
            Loading profile...
          </div>
        ) : null}

        {!profileLoading && !viewUser ? (
          <div className="surface-panel border-[3px] border-neoBorder p-5 text-center text-sm font-black uppercase text-neoMuted shadow-neo">
            Profile not found
          </div>
        ) : null}
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-center">
            <div className="relative w-max">
              <div className="h-28 w-28 overflow-hidden border-[3px] border-neoBorder bg-neoYellow shadow-neo md:h-36 md:w-36 group relative">
                {viewUser?.avatar ? (
                  <img src={`${viewUser.avatar}${viewUser.avatar.includes('dicebear') ? '' : '?tr=w-300,h-300'}`} alt={viewUser.name} className="h-full w-full object-cover" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center bg-neoSurfaceMuted text-2xl font-black">
                    {viewUser?.name?.charAt(0) || viewUser?.username?.charAt(0)}
                  </div>
                )}
            ...
            <div className="flex-1">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-neoMuted">{viewUser?.major}</p>
              <h2 className="mt-2 text-3xl font-black uppercase leading-none md:text-5xl">{viewUser?.name}</h2>
              <p className="mt-2 text-sm font-bold text-neoMuted">@{viewUser?.username}</p>

              <div className="mt-5 max-w-2xl border-[3px] border-dashed border-neoBorder bg-neoSurface p-4 shadow-neo-sm">
                <p className="text-sm font-bold leading-relaxed">{viewUser?.bio}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <StatBox label="Posts" value={profilePosts.length} />
          <StatBox label="Year" value={viewUser?.year ?? "'26"} />
        </section>
        ...
        <section className="surface-panel border-[3px] border-neoBorder p-5 shadow-neo">
          <div className="mb-4 border-b-[3px] border-neoBorder pb-3">
            <h3 className="text-lg font-black uppercase">{isOwnProfile ? 'Your posts' : 'Posts'}</h3>
          </div>

          {postsLoading ? (
            <div className="space-y-4">
              {[1, 2].map((item) => (
                <div key={item} className="h-40 animate-pulse border-[3px] border-neoBorder bg-neoSurfaceMuted shadow-neo-sm" />
              ))}
            </div>
            ...
            <p className="text-center text-sm font-black uppercase text-neoMuted">
              {isOwnProfile ? 'You have not posted yet.' : 'No posts from this student yet.'}
            </p>
          )}
        </section>
        ...
        <section className="surface-panel border-[3px] border-neoBorder p-5 shadow-neo">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {(viewUser?.communities || viewUser?.tribes || []).map((community, index) => (
              <article
                key={community}
